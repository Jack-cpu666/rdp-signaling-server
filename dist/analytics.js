...
exports.analytics = exports.AdvancedAnalytics = void 0;
const events_1 = require("events");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class AdvancedAnalytics extends events_1.EventEmitter {
    ...
    trackSessionStart(sessionId, userId, metadata = {}) {
        this.trackEvent('session_start', {
            sessionId,
            userId
        }, metadata);
    }
    trackSessionEnd(sessionId, duration, dataTransferred) {
    ...
    trackConnection(success, latency, error) {
    ...
    trackFeatureUsage(feature, sessionId, usage = {}) {
    ...
    trackPerformanceMetric(metric, value, unit, context = {}) {
    ...
    aggregateEvents(events, key, period) {
        ...
        for (const event of events) {
            ...
            if (event.metadata.country) {
                aggregation.countries.set(event.metadata.country, (aggregation.countries.get(event.metadata.country) || 0) + 1);
            }
            if (event.metadata.platform) {
                aggregation.platforms.set(event.metadata.platform, (aggregation.platforms.get(event.metadata.platform) || 0) + 1);
            }
        }
        this.aggregatedData.set(key, aggregation);
    }
    getUsageMetrics(period, offset = 0) {
        const now = Date.now();
        let periodMs;
        let key;
        switch (period) {
        ...
        const averageSessionDuration = aggregation.sessionDurations.length > 0
            ? aggregation.sessionDurations.reduce((a, b) => a + b, 0) / aggregation.sessionDurations.length
            : 0;
        ...
        const averageLatency = aggregation.connections.latencies.length > 0
            ? aggregation.connections.latencies.reduce((a, b) => a + b, 0) / aggregation.connections.latencies.length
            : 0;
    ...
    getTrends(metric, period, count = 24) {
        const trends = [];
    ...
    getSystemHealth() {
        ...
        const recentAlerts = this.getAlerts(10).filter(a => Date.now() - a.timestamp < 60 * 60 * 1000 && // Last hour
            !a.resolved);
...
exports.analytics = new AdvancedAnalytics();
